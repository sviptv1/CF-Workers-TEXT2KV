// 定义一个名为 mytxt 的变量，并将 'passwd' 作为默认的读写权限
let mytxt= 'passwd';

export default {
	async fetch (request, env) {
		// 如果环境变量中有 txt，则将其赋值给 mytxt，否则保持默认值
		mytxt = env.txt || mytxt;
		...
		let txt;
		if (url.pathname === `/${mytxt}`){
			txt = mytxt;
		} else {
			// 获取 URL 查询参数中的 'txt'，如果不存在则赋值为 "null"
			txt = url.searchParams.get('txt') || "null";
		}

		// 检查提供的 txt 是否与 mytxt 相符
		if (txt === mytxt) {
			const 文件名 = url.pathname.startsWith('/') ? url.pathname.substring(1) : url.pathname;

			if (文件名 == "config" || 文件名 == mytxt) {
				const html = configHTML(url.hostname, txt);
				...
				return new Response(下载bat(url.hostname, txt), {
				  headers: {
				...
				return new Response(下载sh(url.hostname, txt), {
				  headers: {
		...
		} else {// 如果 txt 不符，返回 'txt 有误'//
			return new Response('txt 有误', {
				status: 400,
				headers: { 'content-type': 'text/plain; charset=utf-8' },
			});
		}
...
function 下载bat(域名,txt) {
	return [
	  ...
	  `set "URL=https://%DOMAIN%/%FILENAME%?txt=%txt%^&b64=%BASE64_TEXT%"`,
	  ``,
	  `rem 显示请求的响应 `,
...
curl -k "https://$DOMAIN/$FILENAME?txt=$txt&b64=$BASE64_TEXT"
echo "更新数据完成"
`
}

function configHTML(域名, txt) {
	return `
	  <html>
		<head>
		...
		<body>
		  ...
		  Windows脚本: <button type="button" onclick="window.open('https://${域名}/config/update.bat?txt=${txt}', '_blank')">点击下载</button>
		  <br>
		  <pre>使用方法: <code>&lt;update.bat&nbsp;ip.txt&gt;</code></pre>
		  <br>
		  Linux脚本: 
		  <code>&lt;curl&nbsp;https://${域名}/config/update.sh?txt=${txt}&nbsp;-o&nbsp;update.sh&nbsp;&&&nbsp;chmod&nbsp;+x&nbsp;update.sh&gt;</code><br>
		  <pre>使用方法: <code>&lt;./update.sh&nbsp;ip.txt&gt;</code></pre><br>
		  <br>
		  在线文档查询: <br>
		  https://${域名}/<input type="text" name="keyword" placeholder="请输入要查询的文档">?txt=${txt}    
		  <button type="button" onclick="window.open('https://${域名}/' + document.querySelector('input[name=keyword]').value + '?txt=${txt}', '_blank')">查看文档内容</button>
		  <button type="button" onclick="navigator.clipboard.writeText('https://${域名}/' + document.querySelector('input[name=keyword]').value + '?txt=${txt}')">复制文档地址</button>
		  </p>
	  <br>
		</body>
	  </html>
